...
void movingai::gm_parser::parse_header(std::fstream &mapfs)
{
	// read header fields
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (mapfs.good())
		{
			...
			{
				std::cerr << "err; map load failed. could not read header." << hfield << std::endl;
				exit(1);
			}
		}
		else
	...
	{
		std::cerr << "err; map type " << this->header_.type_ << "is unknown. known types: octile " << std::endl;
		;
		exit(1);
	}
...
void movingai::gm_parser::parse_map(std::fstream &mapfs)
{
	std::string hfield;
	mapfs >> hfield;
...
void movingai::scenario_manager::load_scenario(const std::string &filelocation)
{
	std::ifstream infile;
...
void movingai::scenario_manager::load_v1_scenario(std::ifstream &infile)
{
	int sizeX = 0, sizeY = 0;
	int bucket;
	std::string map;
